...
/*
 * The initial version has been copied from contrib/pgstattuple in PostgreSQL
 * source tree. We could ask users to install the "pgstattuple" extension, but
 * then we'd have to handle dependencies between various versions of
 * "pgstattuple" and our extension. On the other hand, the copy gives us the
 * possibility to do changes.
 */
#include "postgres.h"
 ...
 */
Datum
squeeze_pgstattuple_approx(PG_FUNCTION_ARGS)
{
